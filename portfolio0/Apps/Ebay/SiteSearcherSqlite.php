	...
	#$db->exec("CREATE TABLE Items(Id STRING PRIMARY KEY, AuctionId STRING UNIQUE, Picture STRING, Link STRING, Title STRING, Location STRING, Type STRING, Price STRING, Condition STRING, Endtime STRING, Seller STRING, Rating STRING, Shipping STRING, Timenum INTEGER, Totalcost STRING, UpdatedTime STRING)");
	...
	if ($file->ack == "Success") {
		...
		foreach($file->searchResult->item as $item) {
			...
			if (substr($price, -3, 1)!='.')
				$price.='0';
			$current = '20'.date("y-m-d\Th:i:s\Z"); 
			$current = str_replace('T','<br>',subStr($current, 0, 19));
			$endtime = $item->listingInfo->endTime;
			$timeNum = strtotime($endtime);
			$endtime = str_replace('T','<br>',subStr($endtime, 0, 19));
			...
			$arr = [(string)$id, (string)$pic, (string)$link, (string)$title, (string)$location, (string)$type, (string)$price, (string)$condition, (string)$endtime, (string)$seller, 
			$stars, (string)$shipping, (integer)$timeNum, (string)$totalcost, (string)$current];
			$storage[] = $arr;
			...
			$db->exec('INSERT OR REPLACE INTO Items (Id, AuctionId, Picture, Link, Title, Location, Type, Price, Condition, Endtime, Seller, Rating, Shipping, Timenum, Totalcost, UpdatedTime) VALUES (NULL, "'.$arr[0].'","'.$arr[1].'","'.$arr[2].'","'.$arr[3].'","'.$arr[4].'","'.$arr[5].'","'.$arr[6].'","'.$arr[7].'","'.$arr[8].'","'.$arr[9].'","'.$arr[10].'","'.$arr[11].'","'.$arr[12].'","'.$arr[13].'","'.$arr[14].'")');
		}
	}
	?>
...
<body onload='tableDisplay();'>
	...
	<form id=itemInput name=itemInput method=get action='SiteSearcherSqlite2.php'>
	...
	<table id ='tabley'>
		<tr id='constant'>
			...
			<td class='cat'><a href='#' onclick='sortByCategory(12);'>Endtime</a></td>
